 ...
 * @author ecolight
 */
public class OnlineTaskJoin extends OnlineTaskBase {
    Gson gson = new Gson();
    
    // シングルトン実装
    private static OnlineTaskJoin instance = null;
    public static final OnlineTaskJoin getInstance(PluginFrame plg_) {
        if (instance == null) {
            instance = new OnlineTaskJoin(plg_);
        }
        return instance;
    }
     ...
     * @param plg_ 
     */
    public OnlineTaskJoin(PluginFrame plg_) {
        super(plg_);
    }
    ...
    public void asyncThread(OnlineThread thread, OnlineDB db, Connection con, OnlinePayload data) throws SQLException {
        ...
        if (conf.getBoolean("OnlinePlayerLogger.Enabled")) {
            // publishの前にDB更新する
            db.updatePlayers(con, data.player);
            con.commit();
            log.log(Level.INFO, "OnlinePlayer Update [{0}]", data.player.getName());
        }

        // ログインのpublishが無効であれば何もしない
        if (conf.getBoolean("Topic.UserLogin.Enable")) {
    ...
    public void mainThread(OnlineThread thread, OnlinePayload data) {
        if (data.result) {
            log.log(Level.INFO, "store online player data [{0}]", data.player.getName());
        } else {
            log.log(Level.INFO, "store online player data failed [{0}]", data.player.getName());
        }
    }
}
